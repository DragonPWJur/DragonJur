...
public final class APIUtils {
    ...
    public static void goToAdminAndChangeChapter1Unit1Text(String testText, boolean isAddWord, APIRequestContext requestContext) {
        String courseId = getActiveCourseId(requestContext);
        String studyGuideId = getStudyGuideIdByCourseId(requestContext,
                courseId);
        JsonObject guideTable = APIServices.getStudyGuideTable(requestContext,
                studyGuideId);
        JsonObject chapter1 = guideTable.getAsJsonArray("chapters")
                .get(0).getAsJsonObject();
        chapter1 = APIServices.getCharterByGuideIdAndChapterId(
                requestContext,
                guideTable.get("id").getAsString(),
                chapter1.get("id").getAsString());

        JsonObject unit1 = chapter1.getAsJsonArray("units")
                .get(0).getAsJsonObject();

        String text = unit1.getAsJsonObject("content")
